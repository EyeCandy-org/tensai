...
void Graphics::drawTexture(std::shared_ptr<Texture> texture, const Transform& transform, const Color& tint) {
...
void Graphics::drawText(const std::string& text, const Vec2& pos, const Color& color) {
...
void Graphics::scale(const Vec2& scale) { camera.scale = camera.scale * Vec2(1.0f/scale.x, 1.0f/scale.y); }